...
public class EnhancedCelestialsCompatibility
{
	...
	public static final void adjustLightmapColors(ClientLevel level, float partialTicks, float skyDarken, float skyLight, float blockLight, int pixelX, int pixelY, Vector3f colors)
    {
		...
		if(darkerWorld)
		{
			...
			skyVector.lerp(new Vector3f(1.0F, 1.0F, 1.0F), 0.35F);
			...
			else
			{
				...
				Vector3f vector3f2 = new Vector3f(skyVector.x(), skyVector.y(), skyVector.z());
				vector3f2.mul(naturalLight);
				...
				if(minecraft.gameRenderer.getDarkenWorldAmount(partialTicks) > 0.0F)
				{
					...
					Vector3f vector3f3 = new Vector3f(lightColor.x(), lightColor.y(), lightColor.z());;
					vector3f3.mul(0.7F, 0.6F, 0.6F);
					lightColor.lerp(vector3f3, f12);
				}
			}
			
			float[] set = new float[] {lightColor.x(), lightColor.y(), lightColor.z()};
			colors.set(set);
		}
	}
}

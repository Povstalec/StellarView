...
public class StarField extends SpaceObject
{
	...
	protected MeshData generateDustCloudBuffer(Tesselator tesselator)
	{
		RandomSource randomsource = RandomSource.create(seed);
			...
			Vector3f parentVector, AxisRotation parentRotation)
	{
		...
		if(!GeneralConfig.disable_stars.get() && starBrightness > 0.0F)
		{
			...
			this.starBuffer.drawWithShader(transformedModelView, projectionMatrix, difference, hasTexture ? StellarViewShaders.starTexShader() : StellarViewShaders.starShader());
			VertexBuffer.unbind();

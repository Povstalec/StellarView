...
public class ViewCenter
{
	...
	protected boolean renderSkyObjectsFrom(ClientLevel level, Camera camera, float partialTicks, PoseStack stack, Matrix4f projectionMatrix, Runnable setupFog, BufferBuilder bufferbuilder)
	{
		...
		if(!GeneralConfig.disable_view_center_rotation.get())
		{
			...
			stack.mulPose(Vector3f.YP.rotation((float) getAxisRotation().yAxis()));
			stack.mulPose(Vector3f.ZP.rotation((float) getAxisRotation().zAxis()));
			stack.mulPose(Vector3f.XP.rotation((float) getAxisRotation().xAxis()));
			
			stack.mulPose(Vector3f.YP.rotation((float) rotation));
			stack.mulPose(Vector3f.ZP.rotation((float) getZRotation(level, camera, partialTicks)));
		}

...
import net.povstalec.stellarview.mixin.BufferBuilderMixin;
...
public abstract class StarData
{
	...
	public static void addStarHeightWidthSizeDistance(BufferBuilder builder, float height, float width, float size, float distance)
	{
		long i = ((BufferBuilderMixin) builder).invokeBeginElement(StellarViewVertexFormat.ELEMENT_HEIGHT_WIDTH_SIZE_DISTANCE);
		...
		if (i != -1L)
		{
			...
			MemoryUtil.memPutFloat(i + Float.BYTES * 3, distance);
		}
	}
	...
	public static class LOD
	{
		...
		public void createStar(BufferBuilder builder, boolean hasTexture, int i)
		{
			...
			for(int j = 0; j < 4; ++j)
			{
				...
				builder.addVertex((float) starCoords[3 * i], (float) starCoords[3 * i + 1], (float) starCoords[3 * i + 2])
						.setColor((byte) starRGBA[4 * i], (byte) starRGBA[4 * i + 1], (byte) starRGBA[4 * i + 2], (byte) starRGBA[4 * i + 3]);
				
				addStarHeightWidthSizeDistance(builder, (float) height, (float) width, hasTexture ? (float) starSizes[i] * 4F : (float) starSizes[i], (float) starDistances[i]);
		...
		{
			final var bufferBuilder = tesselator.begin(VertexFormat.Mode.QUADS, hasTexture ? StellarViewVertexFormat.STAR_POS_COLOR_LY_TEX : StellarViewVertexFormat.STAR_POS_COLOR_LY);
			...
			for(int j = 0; j < 4; ++j)
			{
				...
				if(hasTexture)
				{
					builder.addVertex((float) (starX + projectedX), (float) (starY + heightProjectionY), (float) (starZ + projectedZ))
							...
							.setColor((byte) starRGBA[4 * i], (byte) starRGBA[4 * i + 1] , (byte) starRGBA[4 * i + 2], alpha);
				}
				else
				{
					builder.addVertex((float) (starX + projectedX), (float) (starY + heightProjectionY), (float) (starZ + projectedZ))
